...
class CNNSizeHelper(object):
    ...
    def compute_inside_neuron(self, layer_name):
        """
            Compute inside neuron range
            Inside neurons' receptive fields are in the image wholly.
        :type projection: util.CaffeProjection
        :param projection: projection for a convolutional network
        :type layer_name: string
        :param layer_name: layer's name
        :rtype: tuple
        :return: ranges for inside neurons
        """
        layer_info_dict_this = self.layer_info_dict[layer_name]
    ...
    def compute_minimum_coverage(self, layer_name, top_left, bottom_right):
        ...
        """
        top_pos, left_pos = top_left
        bottom_pos, right_pos = bottom_right
        assert 0 <= top_pos and top_pos <= bottom_pos and bottom_pos <= self.input_size[0]
        assert 0 <= left_pos and left_pos <= right_pos and right_pos <= self.input_size[1]
        ...
        max_r_all = stride_g * row_idx - pad_g + kernelsize_g - 1

        min_c_all = stride_g * col_idx - pad_g
        max_c_all = stride_g * col_idx - pad_g + kernelsize_g - 1
        ...
        top_left_location = row_idx[mask_topleft].max(), col_idx[mask_topleft].max()
        assert mask_topleft[top_left_location]
        ...
        bottom_right_location = row_idx[mask_bottomright].min(), col_idx[mask_bottomright].min()
        assert mask_bottomright[bottom_right_location]
        return top_left_location, bottom_right_location
