...
class V1S(object):
    ...
    def generate_repr(self, img_fname, model):
        return self._generate_repr(img_fname, model)
    ...
    def _part_generate_repr(self, img_fname, params, featsel):
        ...
        if imga0.ndim == 3:
            ...
            imga0 = 0.2989*imga0[:,:,0] + \
                    0.5870*imga0[:,:,1] + \
                    0.1140*imga0[:,:,2]
        print "imga0, mean {}, std {}".format(imga0.mean(), imga0.std())
        
        imga00 = imga0.copy() # save a copy
        # smoothing
        ...
        print "imga1, mean {}, std {}".format(imga1.mean(), imga1.std())
        

        # Here, check parameters in featsel to see if we are done here.
        if 'raw_image' in featsel:
            im_type = featsel['raw_image']
            if im_type == '00':
                return imga00.ravel() # image before smoothing.
            elif im_type == '0':
                return imga0.ravel() # image after smoothing.
            elif im_type == '1':
                return imga1.ravel()
            else:
                raise NotImplementedError('do not have this kind raw image: {}'.format(im_type) )
        ...
        print "imga2, mean {}, std {}".format(imga2.mean(), imga2.std())
        ...
        print "imga3, mean {}, std {}".format(imga3.mean(), imga3.std())

        # -- 4. output normalization
        imga4 = v1s_norm(imga3, **params['normout'])
        print "imga4, mean {}, std {}".format(imga4.mean(), imga4.std())

        # -- 5. volume dimension reduction
        imga5 = v1s_dimr(imga4, **params['dimr'])
        output = imga5
        print "output, mean {}, std {}".format(output.mean(), output.std())
    ...
    def _get_gabor_filters(self, params):
        ...
        if self._filt_l is None:        
            ...
            if 'LCADictFile' not in params:
                ...
                # load model file.
                LCAmodel = sio.loadmat(params['LCADictFile'])
                # LCADictFile specifies which dict in the file to use.
                filtAll = LCAmodel['v1sFilterArray'][params['LCADictIdx'],0]

                for iFilt in range(filtAll.shape[2]):
                    # create 2d gabor
                    filt = filtAll[:,:,iFilt]
